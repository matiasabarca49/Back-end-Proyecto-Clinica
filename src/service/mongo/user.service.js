...
export default class UsersService {
    ...
    async createUser(newUser, userSession) {
        ...
        // 1️⃣ Crear el usuario principal
        ...
        const userDoc = userAdded.dt;

        // 2️⃣ Si es Doctor, crear registro en colección Doctor
        const isDoctor = String(userNormalized.rol || "").toLowerCase() === "doctor";
        if (isDoctor) {
            const doctorData = this._extractDoctorData(newUser, userDoc);
            
            // Validar campos requeridos
            const validation = this._validateDoctorFields(doctorData);
            if (!validation.valid) {
                // Compensación: eliminar el usuario creado
                await this._safeDeleteUser(userDoc._id);
                return {
                    status: false,
                    error: {
            ...
            const doctorAdded = await persistController.createDocument(Doctor, doctorData);
        ...
        // 3️⃣ Éxito
        return { ...userAdded, dt: sendUserFormated(userDoc) };
    }
    ...
    async updateUser(userID, toUpdate, userSession) {
        try {
            ...
            const prevUser = await this.getAllUserById(userID);
            if (!prevUser) return false;

            // 2️⃣ Determinar roles
            ...
            // 3️⃣ Actualizar usuario principal
            ...
            // Mapear errores de duplicados
            if (error?.code === 11000) {
                return {
                    status: false,
                    error: { code: 11000, message: "Email duplicado" }
                };
            }
    ...
    async deleteUser(userID, userSession) {
        try {
            // 1️⃣ Obtener usuario antes de eliminarlo
            const user = await this.getAllUserById(userID);
            if (!user) return false;

            // 2️⃣ Si es Doctor, eliminar de la colección Doctor
